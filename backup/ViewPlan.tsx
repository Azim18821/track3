                      ...
                      return (
                        <div key={day} className="space-y-4">
                          ...
                          <div className="space-y-6">
                            ...
                            {["breakfast", "pre_workout", "lunch", "post_workout", "dinner"].map(
                              (mealType) =>
                                meals[mealType] && (
                              ...
                              Object.entries((mealPlan.shoppingList as any).categories).map(
                                ([category, items]: [string, any]) => (
                                  <div key={category} className="mb-4">
                                    <Collapsible
                                      defaultOpen={true}
                                      className="rounded-md border dark:border-gray-700 collapsible"
                                    >
                                      <CollapsibleTrigger className="flex w-full items-center justify-between p-4 bg-slate-50 dark:bg-gray-800 rounded-t-md hover:bg-slate-100 dark:hover:bg-gray-700 transition-colors">
                                        <div className="flex items-center gap-3">
                                          {category === 'Produce' && <Salad className="h-4 w-4 text-green-500" />}
                                          {category === 'Meat & Seafood' && <Beef className="h-4 w-4 text-red-500" />}
                                          {category === 'Dairy & Eggs' && <Milk className="h-4 w-4 text-blue-500" />}
                                          {category === 'Grains & Bread' && <Wheat className="h-4 w-4 text-amber-500" />}
                                          {category === 'Canned Goods' && <Package className="h-4 w-4 text-gray-500" />}
                                          {category === 'Frozen' && <Snowflake className="h-4 w-4 text-cyan-500" />}
                                          {category === 'Pantry' && <Package2 className="h-4 w-4 text-orange-500" />}
                                          {category === 'Spices & Herbs' && <FlaskConical className="h-4 w-4 text-purple-500" />}
                                          {category === 'Beverages' && <Wine className="h-4 w-4 text-indigo-500" />}
                                          {category === 'Other' && <ShoppingBag className="h-4 w-4 text-gray-500" />}
