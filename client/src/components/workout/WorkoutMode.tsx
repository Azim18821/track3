...
const WorkoutMode: React.FC<WorkoutModeProps> = ({ workout, onExit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-background z-50 flex flex-col h-full overflow-hidden workout-modal-container safe-area-left safe-area-right">
      ...
      {/* Main content - scrollable with improved styling */}
      <div className="flex-1 overflow-y-auto px-3 sm:px-4 pt-4 pb-20 workout-modal-content bg-slate-50 dark:bg-slate-900">
        {/* Exercise navigation pills - styled for better readability */}
          ...
          {workoutState.exercises.map((exercise, index) => {
            ...
            return (
              <Button
                ...
                size="sm"
                className={`flex-shrink-0 rounded-full px-3 shadow-sm border ${
                  exerciseCompleted 
                    ...
                    : exerciseInProgress 
                      ...
                      : activeExerciseIndex === index
                        ? "bg-blue-600 text-white border-blue-500 hover:bg-blue-700" // Enhanced primary styling
        ...
        {/* Active exercise - Enhanced card design */}
        <Card className="mb-6 shadow-md border-none overflow-hidden bg-white dark:bg-slate-800">
          <div className="bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-800/50 px-4 py-3 flex justify-between items-center">
            ...
            <Button 
              variant="outline" 
              size="sm" 
              className="h-8 px-3 text-xs flex items-center gap-1.5 border-blue-200 dark:border-blue-700 bg-white dark:bg-slate-700 hover:bg-blue-50 dark:hover:bg-slate-600"
          ...
          <div className="px-4 py-3">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
              <div className="flex items-center gap-1.5 text-sm bg-blue-50 dark:bg-blue-900/30 px-2.5 py-1.5 rounded-md">
                <Dumbbell className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                <span>{activeExercise.sets} sets × {activeExercise.reps || '?'} reps</span>
              </div>
              
              {typeof activeExercise.weight === 'number' && (
                <Badge variant="outline" className="font-normal px-3 py-1.5 h-7 border-blue-200 dark:border-blue-700">
                  {activeExercise.weight} {activeExercise.unit || 'kg'}
                </Badge>
              )}
              
              <div className="hidden sm:flex items-center gap-1.5 text-xs bg-amber-50 dark:bg-amber-900/20 px-2.5 py-1.5 rounded-md">
          ...
          <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10 border-t border-blue-100 dark:border-blue-800/30">
            {/* Rest timer buttons - Enhanced design */}
            <div className="grid grid-cols-3 gap-3">
              <Button 
                variant="outline"
                className="h-11 sm:h-12 bg-white dark:bg-slate-800 border-blue-200 dark:border-blue-700 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-slate-700 shadow-sm"
                onClick={() => startRestTimer(60)}
              >
                <TimerIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1.5 sm:mr-2" />
                <span className="hidden sm:inline">60s Rest</span>
                <span className="sm:hidden">60s</span>
              </Button>
              <Button 
                variant="outline"
                className="h-11 sm:h-12 bg-white dark:bg-slate-800 border-amber-200 dark:border-amber-700/70 text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-slate-700 shadow-sm"
              ...
              <Button 
                variant="outline"
                className="h-11 sm:h-12 bg-white dark:bg-slate-800 border-purple-200 dark:border-purple-700/70 text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-slate-700 shadow-sm"
          ...
          <CardContent className="pt-5">
            ...
            {/* Enhanced Info message for plan mode */}
            {isPlanModeWorkout && (
              <div className="mb-5 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-lg text-sm flex items-start shadow-sm">
                <InfoIcon className="h-4 w-4 mt-0.5 mr-3 flex-shrink-0 text-blue-600 dark:text-blue-400" />
                <span className="text-blue-800 dark:text-blue-200">Enter your actual weight and reps for each set. Mark sets as completed as you perform them.</span>
              </div>
            )}

            {/* Sets tracking with enhanced visual design */}
            <div className="space-y-4">
              {activeExercise.setsData?.map((setData, setIndex) => (
                <div key={setIndex} className={`border rounded-xl overflow-hidden transition-all shadow-sm ${
                  setData.completed 
                    ? "border-green-300 dark:border-green-700 bg-gradient-to-r from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-900/30"
                  ...
                  <div className={`flex justify-between items-center px-4 py-3 border-b ${
                    setData.completed 
                      ? 'bg-gradient-to-r from-green-100 to-green-50 dark:from-green-900/30 dark:to-green-900/20 border-green-200 dark:border-green-800/50' 
                      : 'bg-gradient-to-r from-slate-50 to-white dark:from-slate-800 dark:to-slate-800/80 border-slate-100 dark:border-slate-700/50'
                    }`}>
                    <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      className={`h-8 text-xs shadow-sm ${
                        setData.completed 
                          ...
                          : "bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-blue-300 dark:hover:border-blue-600"
                      }`}
                  ...
                  <div className="p-4 grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={`h-10 border-slate-200 dark:border-slate-600 focus:ring-blue-500 ${
                          setData.completed ? "bg-white/90 dark:bg-slate-700/90" : ""
                        }`}
              ...
              {/* Add Extra Set Button - Enhanced design */}
              <Button 
                variant="outline" 
                className="w-full flex items-center justify-center gap-2 py-3 h-auto border-dashed border-2 mt-4 border-blue-200 dark:border-blue-800/70 text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 shadow-sm"
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <Button 
            variant="outline" 
            className="flex items-center justify-center gap-3 h-16 text-base transition-all border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 group"
            onClick={() => {
          ...
          >
            <div className="flex items-center justify-center h-10 w-10 rounded-full bg-slate-100 dark:bg-slate-700 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/30 transition-colors">
              <ChevronLeft className="h-5 w-5 text-slate-600 dark:text-slate-300 group-hover:text-blue-600 dark:group-hover:text-blue-400" />
            </div>
            
            <div className="flex flex-col items-start">
              <span className="text-xs text-slate-500 dark:text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400">Previous Exercise</span>
              <span className="font-medium truncate max-w-[100px] text-slate-800 dark:text-slate-200">
          ...
          <Button 
            variant={activeExerciseIndex < workoutState.exercises.length - 1 ? "default" : "outline"}
            className={`flex items-center justify-center gap-3 h-16 text-base transition-all shadow-sm ${
