...
export function OnboardingLayout({ children }: OnboardingLayoutProps) {
  ...
  return (
    <div className={`ios-screen-container w-full bg-gradient-to-b from-blue-50 to-white dark:from-slate-900 dark:to-background flex flex-col overflow-x-hidden
      ${isIOSStandalone ? 'ios-standalone-mode h-auto' : 'min-h-screen'}`}>
      ...
      {/* Main content area with safe area padding when in standalone mode - reduced height */}
      <main className={`flex justify-center px-1 sm:px-4 
        ${isIOSStandalone ? 'pt-0 pb-0 h-auto' : 'flex-grow py-2 sm:py-4 items-start sm:items-center'}`}>
        {children}
      </main>
      
      {/* Footer with proper bottom spacing */}
      <footer className={`text-center text-xs text-muted-foreground
        ${isIOSStandalone ? 'pb-2 pt-0' : 'py-2 sm:py-3'}`}>
