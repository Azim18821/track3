...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Debug endpoint to help diagnose workout issues
  app.get("/api/debug/workouts", ensureAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = req.user!.id;
      
      // Get all workouts in the system
      const allWorkoutsInDb = await db.select().from(workouts);
      
      // Get workouts for the current user
      const userWorkouts = allWorkoutsInDb.filter(w => w.userId === userId);
      
      // Check if client has any trainer
      const clientTrainers = await db
        .select()
        .from(trainerClients)
        .where(eq(trainerClients.clientId, userId));
      
      // Get workout count created by createClientWorkoutsFromPlan function (by checking notes)
      const autoGeneratedWorkouts = userWorkouts.filter(w => 
        w.notes && w.notes.includes('Auto-generated from fitness plan'));
      
      res.json({
        userId,
        totalWorkoutsInSystem: allWorkoutsInDb.length,
        userWorkoutCount: userWorkouts.length,
        hasTrainers: clientTrainers.length > 0,
        trainerCount: clientTrainers.length,
        autoGeneratedWorkoutCount: autoGeneratedWorkouts.length,
        sample: userWorkouts.slice(0, 3) // First few user workouts
      });
    } catch (error) {
      console.error("Error in debug endpoint:", error);
      res.status(500).json({ message: "Error in debug workouts endpoint" });
    }
  });
